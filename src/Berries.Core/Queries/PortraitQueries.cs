...
public sealed class PortraitQueries(IFileSystem fileSystem)
{
        ...
        CancellationToken cancellationToken = default)
    {
        return Task.Run<IReadOnlyList<FileInstance>>(() =>
        {
            ...
            progress?.Report(new OperationProgress("Finding duplicate files in directory", 0, sets.Count));
            ...
            for (var i = 0; i < sets.Count; i++)
            {
                ...
                if ((i & 0xff) == 0 || i + 1 == sets.Count)
                    progress?.Report(new OperationProgress("Finding duplicate files in directory", i + 1, sets.Count));
            }
