...
public sealed record BranchPairCandidate(
    BranchRecord Branch,
...
public sealed record BranchPairSuggestion(
    ...
    IReadOnlyList<BranchPairCandidate> Candidates,
    int CandidateSeedRank = 0);

public sealed record BranchPairAnalysisResult(
...
public sealed class BranchPairAnalyzer(IFileSystem fileSystem)
{
    private const int CandidateSeedLimit = 10;

    public BranchPairAnalysisResult Analyze(
        Corpus corpus,
        ...
        IProgress<OperationProgress>? progress = null)
    {
        ...
        progress?.Report(new OperationProgress("Finding promising Branch Pairs"));
        var selected = new List<BranchPairSuggestion>();
        var blockedRoots = new List<FileSystemPath>();

        while (selected.Count < suggestionLimit)
        {
            cancellationToken.ThrowIfCancellationRequested();
            ...
            for (var rank = 0; rank < candidateSeeds.Length; rank++)
            {
                ...
                if (candidates.Count > 0)
                    suggestions.Add(new BranchPairSuggestion(seed, candidates, rank + 1));
            }

            if (suggestions.Count == 0) break;
            var winner = suggestions
                .OrderByDescending(item => item.Candidates[0].Score)
                .ThenByDescending(item => item.Seed.ExcessConcentratedGroups)
                .ThenBy(item => item.CandidateSeedRank)
                .First();

            var diagnosedCandidates = winner.Candidates.Select(candidate => candidate with
            {
                DirectDirectoryPairSharedGroupCount = directDirectoryPairSharedGroups.GetValueOrDefault(
                    (winner.Seed.Branch.Path, candidate.Branch.Path))
            }).ToArray();

            winner = winner with { Candidates = diagnosedCandidates };
            selected.Add(winner);
            blockedRoots.Add(winner.Seed.Branch.Path);
            blockedRoots.Add(winner.Candidates[0].Branch.Path);
            progress?.Report(new OperationProgress($"Finding promising Branch Pairs — {selected.Count:N0} found"));
        }

        timer.Stop();
        return new BranchPairAnalysisResult(selected, timer.Elapsed);
    }
    ...
    private IReadOnlyList<BranchPairCandidate> FindCandidates(
        BranchPriorityMetric seed,
        ...
        int limit)
    {
        ...
        return overlaps
            ...
            .Select(item => CreateCandidate(seed, byPath[item.Key], item.Value))
