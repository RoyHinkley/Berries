...
/// <summary>Computes inexpensive branch-local statistics independently of BranchPair enumeration.</summary>
public sealed class BranchStatisticsAnalyzer(IFileSystem fileSystem)
{
    public BranchStatisticsResult Analyze(Corpus corpus, Portrait portrait, IReadOnlyList<DuplicateSet> duplicateSets,
        DuplicateSettlements settlements, IReadOnlyList<DirectoryRecord> duplicateDirectories,
        CancellationToken cancellationToken = default, IProgress<OperationProgress>? progress = null)
    {
        ...
        var totalWork = (long)physicalDirectories.Length + duplicateDirectories.Count + duplicateSets.Count;
        long completed = 0;
        progress?.Report(new OperationProgress("Analyzing branches", completed, totalWork));

        foreach (var group in physicalDirectories)
        {
            ...
            foreach (var branch in GetAncestorsWithinCorpus(group.Key, corpus, ancestorsByDirectory))
            {
                ...
                accumulator.FileCount += fileCount; accumulator.DirectoryCount++;
            }
            progress?.Report(new OperationProgress("Analyzing branches", ++completed, totalWork));
        }
        ...
        foreach (var directory in duplicateDirectories)
        {
            ...
            {
                var accumulator = GetAccumulator(accumulators, branch);
                accumulator.DuplicateFileCount += directory.DuplicateFileCount; accumulator.DuplicateDirectoryCount++;
            }
            progress?.Report(new OperationProgress("Analyzing branches", ++completed, totalWork));
        }
        ...
        foreach (var duplicateSet in duplicateSets)
        {
            ...
            if (settlements.HasUnresolvedRelationship(duplicateSet))
            {
                ...
                foreach (var directory in participatingDirectories)
                    foreach (var branch in GetAncestorsWithinCorpus(directory, corpus, ancestorsByDirectory)) touchedBranches.Add(branch);
                foreach (var branch in touchedBranches) GetAccumulator(accumulators, branch).DuplicateContentCount++;
            }
            progress?.Report(new OperationProgress("Analyzing branches", ++completed, totalWork));
        }

        var branches = accumulators.Where(item => item.Value.DuplicateContentCount > 0).Select(item =>
        {
            var parent = fileSystem.GetParentDirectory(item.Key);
            if (parent is not null && !accumulators.ContainsKey(parent.Value)) parent = null;
            return new BranchRecord(item.Key, parent, item.Value.FileCount, item.Value.DirectoryCount,
                item.Value.DuplicateFileCount, item.Value.DuplicateContentCount, item.Value.DuplicateDirectoryCount);
        }).OrderByDescending(branch => branch.DuplicateContentCount).ThenByDescending(branch => branch.DuplicateFileCount)
          .ThenBy(branch => branch.Path.Value, StringComparer.Ordinal).ToArray();
        timer.Stop();
        return new BranchStatisticsResult(branches, timer.Elapsed);
    }

    private static IReadOnlyCollection<FileSystemPath> GetUnresolvedParticipatingDirectories(DuplicateSet duplicateSet, DuplicateSettlements settlements)
    {
        if (settlements.AcceptedPairCount == 0) return duplicateSet.Files.Select(file => file.ParentDirectory).Distinct().ToArray();
        var directories = new HashSet<FileSystemPath>(); var files = duplicateSet.Files;
        ...
        {
            if (settlements.IsPairAccepted(duplicateSet.Content, files[firstIndex], files[secondIndex])) continue;
            directories.Add(files[firstIndex].ParentDirectory); directories.Add(files[secondIndex].ParentDirectory);
        }
        return directories;
    }
    ...
    private static Accumulator GetAccumulator(IDictionary<FileSystemPath, Accumulator> accumulators, FileSystemPath path)
    {
        if (!accumulators.TryGetValue(path, out var accumulator)) { accumulator = new Accumulator(); accumulators[path] = accumulator; }
        return accumulator;
    }
