...
public sealed class GuiController
{
    ...
    public async Task<ScanResult> ScanAsync(IEnumerable<string> rootPaths, Func<FileSystemPath, bool>? excludePath = null,
        ...
        IProgress<OperationProgress>? analysisProgress = null, CancellationToken cancellationToken = default)
    {
        ...
        Debug.WriteLine("[Berries] Acquiring initial portrait..."); phaseTimer.Restart();
        ...
        var contentsByPath = duplicates.DuplicateSets.SelectMany(set => set.Files.Select(file => (file.Path, set.Content))).ToDictionary(item => item.Path, item => item.Content);
        var sessionPortrait = new Portrait(duplicates.Portrait.Files.Select(file => contentsByPath.TryGetValue(file.Path, out var content) ? file with { Content = content } : file));
        ...
        Debug.WriteLine("[Berries] Scan and analysis ready."); return Scan;
    }

    public Task RefreshAnalysisAsync(CancellationToken cancellationToken = default) => RefreshAnalysisAsync(null, cancellationToken);

    public async Task RefreshAnalysisAsync(IProgress<OperationProgress>? progress, CancellationToken cancellationToken = default)
    {
