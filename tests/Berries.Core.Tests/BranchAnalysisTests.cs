...
public sealed class BranchAnalysisTests
{
    [Fact]
    public async Task AnalyzeBranchesAsync_AggregatesDescendantDirectoryPairEvidence()
    {
        ...
        var result = await engine.AnalyzeBranchesAsync(
            new Corpus(new[] { new CorpusRoot(root) }),
            directoryPairs);

        var pair = Assert.Single(result.BranchPairs,
    ...
    [Fact]
    public async Task AnalyzeBranchesAsync_UsesDirectoryPairWeights()
    {
        var root = Path(@"X:\Corpus");
        var a = Path(@"X:\Corpus\A");
        var b = Path(@"X:\Corpus\B");
    ...
    [Fact]
    public async Task AnalyzeBranchesAsync_NestedRootsExcludeDescendantSubtreeFromAncestorSide()
    {
        var root = Path(@"X:\Corpus");
    ...
    [Fact]
    public async Task SettledWholeDuplicateSet_DoesNotPropagateIntoBranchPairs()
    {
        var root = Path(@"X:\Corpus");
        var a = Path(@"X:\Corpus\A");
        var b = Path(@"X:\Corpus\B");
        ...
        var branchAnalysis = await engine.AnalyzeBranchesAsync(
            new Corpus(new[] { new CorpusRoot(root) }),
            directoryAnalysis.DirectoryPairs);

        Assert.DoesNotContain(branchAnalysis.BranchPairs,
            pair => pair.FirstRoot == a && pair.SecondRoot == b);
        Assert.Contains(branchAnalysis.BranchPairs,
    ...
    private sealed class TestFileSystem : IFileSystem
    {
        ...
        private static InvalidOperationException UnexpectedCall() =>
            new("Branch analysis unexpectedly performed file I/O.");
    }
}
