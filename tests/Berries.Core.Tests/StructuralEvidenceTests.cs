...
{
    [Fact]
    public void AnalyzeScopePair_FindsContributingPairs_Coverage_AndSubsidiaries()
    {
        ...
        var b1 = Path(@"X:\Corpus\B\One");
        var b2 = Path(@"X:\Corpus\B\Two");

        var fA1 = File(a1, "a1");
        var fB1 = File(b1, "b1");
        var fA2 = File(a2, "a2");
        var fB2 = File(b2, "b2");
        var onlyA = File(a, "only-a");

        var duplicateSets = new[]
        {
            new DuplicateSet(new ContentId("01"), new[] { fA1, fB1 }),
            new DuplicateSet(new ContentId("02"), new[] { fA2, fB2 }),
            new DuplicateSet(new ContentId("03"), new[] { onlyA, File(a, "only-a-copy") })
        };
        var directoryPairs = new[]
        {
            new DirectoryPair(a1, b1, 1),
            new DirectoryPair(a2, b2, 1)
        };
        var parent = new ScopePair(a, b, 2, 2);
        var child = new ScopePair(a1, b1, 1, 1);
        var other = new ScopePair(a1, a2, 1, 1);

        var evidence = analyzer.AnalyzeScopePair(
            parent,
            duplicateSets,
            directoryPairs,
            new[] { parent, child, other });

        Assert.False(evidence.RootsNested);
        Assert.Equal(3, evidence.FirstSideDuplicateContentCount);
        Assert.Equal(2, evidence.SecondSideDuplicateContentCount);
        Assert.Equal(1, evidence.SubsidiaryScopePairCount);
        Assert.Equal(child, Assert.Single(evidence.StrongestSubsidiaryScopePairs));
    ...
    public void AnalyzeScopePair_RecognizesNestedRoots_AndExcludesDescendantSubtreeFromAncestorSide()
    {
        ...
        var outside = Path(@"X:\Corpus\Parent\Outside");
        var inside = Path(@"X:\Corpus\Parent\Child\Inside");

        var crossing = new DuplicateSet(
            new ContentId("cross"),
            new[] { File(outside, "a"), File(inside, "b") });
        var internalChild = new DuplicateSet(
            new ContentId("internal"),
            new[] { File(inside, "c"), File(inside, "d") });
        var pair = new ScopePair(parent, child, 1, 1);

        var evidence = analyzer.AnalyzeScopePair(
            pair,
            new[] { crossing, internalChild },
            new[] { new DirectoryPair(outside, inside, 1) },
            new[] { pair });

        Assert.True(evidence.RootsNested);
        Assert.Equal(1, evidence.FirstSideDuplicateContentCount);
        Assert.Equal(2, evidence.SecondSideDuplicateContentCount);
        Assert.Empty(evidence.StrongestSubsidiaryScopePairs);
        Assert.Single(evidence.StrongestContributingDirectoryPairs);
    }
    ...
    private sealed class TestFileSystem : IFileSystem
    {
        ...
        private static InvalidOperationException UnexpectedCall() =>
            new("Structural evidence analysis unexpectedly performed file I/O.");
    }
}
