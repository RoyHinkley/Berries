...
{
    [Fact]
    public async Task AnalyzeDirectoriesAsync_BuildsDirectRecords_AndRanksPairsBySharedGroups()
    {
        ...
        var groups = new[]
        {
            new Group(new ContentId("01"), new[] { a1, b1, c1 }),
            new Group(new ContentId("02"), new[] { a2, b2, b2Copy }),
        ...
        Assert.Equal(2, result.DirectoryPairs[0].SharedGroupCount);
        Assert.Equal(directoryA, result.DirectoryPairs[0].First);
        Assert.Equal(directoryB, result.DirectoryPairs[0].Second);
        Assert.All(result.DirectoryPairs.Skip(1), pair => Assert.Equal(1, pair.SharedGroupCount));

        Assert.Equal(3, result.Graph.TotalDirectoryCount);
        Assert.Equal(3, result.Graph.GroupedDirectoryCount);
        Assert.Equal(2, result.Graph.InternalGroupDirectoryCount);
        ...
        Assert.Equal(2, nodeA.MaxSharedGroupCount);
        Assert.Equal(1.5, nodeA.MeanSharedGroupCount);
